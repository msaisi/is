...
class Gradstate_institutions_auth extends Fuel_base_library {
	...
	{
		$valid_user = $this->CI->gradstate_institutions_model->valid_user(trim($user), trim($pwd));
		if (!empty($valid_user)) 
		{
			...
			$updated_where = array('institution_id' => decrypt($valid_user['institution_id']), 'is_active' => 'yes');
			$email=decrypt($valid_user['email']);
			// update salt on login
			if ($this->CI->gradstate_institutions_model->update($updated_user_profile, $updated_where))
			{
				$this->set_valid_user($valid_user);
				$this->CI->fuel->logs->write(lang('auth_institution_log_login_success',$email, $this->CI->input->ip_address()), 'debug');
				return TRUE;
			}
			else
			{
	...
	/**
	 * Returns either an array of the logged in institutions session data or a single value of the data if a $key parameter is passed
	 *
	 * @access	public
	...
	{
		$key = 'gradstate_institution_'.md5(FCPATH); // unique to the site installation
		if (isset($this->CI->session))
		{
	...
	/**
	 * Returns whether the browsers institutions remote address matches 
	 *
	 * @access	public
	...
	/**
	 * Returns the currently logged in institutions language preference
	 *
	 * @access	public
	 * @return	string
	 */	
	...
	public function logout()
	{
		...
		// remove UI cookie
		$ui_cookie_name = 'gradstate_institution_ui_'.str_replace('gradstate_institution_', '', $this->get_fuel_trigger_cookie_name());
		$config = array(
			'name' => $ui_cookie_name,
			'path' => WEB_PATH
		);
