...
class Companies_users_auth extends Fuel_base_library {
	...
	{
		$valid_user = $this->CI->companies_users_model->valid_user(trim($user), trim($pwd));
		if (!empty($valid_user)) 
		{
			...
			$updated_where = array('email' => decrypt($valid_user['email']), 'is_active' => 'yes');
			// update salt and last login on login
			if ($this->CI->companies_users_model->update($updated_user_profile, $updated_where))
			{
	...
	{
		$valid_user = $this->CI->companies_users_model->unlock_account(trim($user), trim($pwd));
		if (!empty($valid_user)) 
		{
			return TRUE;
		}
	...
	{
		$key = 'company_users_'.md5(FCPATH); // unique to the site installation
		if (isset($this->CI->session))
		{
	...
	public function logout()
	{
		...
		// remove UI cookie
		$ui_cookie_name = 'company_users_ui_'.str_replace('company_users_', '', $this->get_fuel_trigger_cookie_name());
		$config = array(
			'name' => $ui_cookie_name,
			'path' => WEB_PATH
		);
