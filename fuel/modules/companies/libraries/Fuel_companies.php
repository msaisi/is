...
class Fuel_companies extends Fuel_advanced_module {
	...
	function __construct($params = array())
	{
		...
		if (empty($params))
		{
			$params['name'] = 'companies';
		}
		$this->initialize($params);

		$this->from_email="no-reply@companies.com";
	}
	...
	function initialize($params = array())
	{
		...
		if (!empty($params))
		{
			...
			{
				$sans_companies_key = substr($key, count('companies_'));
				if (isset($this->$key))
				{
					$this->$sans_companies_key = $val;
				}
			}
		}		
	}
	...
	/**
	 * Returns the title of the companies specified in the settings
	 *
	 * @access	public
	 * @return	string
	 */
	...
	/**
	 * Returns the descripton of the companies specified in the settings
	 *
	 * @access	public
	 * @return	string
	 */
	...
	/**
	 * Returns the domain to be used for the companies based on the FUEL configuration. 
	...
	/**
	 * Returns header of the companies
	 *
	 * @access	public
	 * @param	string
	 * @return	string
	...
	/**
	 * Returns a view for the companies
	 *
	 * @access	public
	 * @param	string
	 * @param	array
	...
	/**
	 * Returns a block view file for the companies
	 *
	 * @access	public
	 * @param	string
	 * @param	array
	...
	/**
	 * Returns a the specified companies model object. Options are posts, categories, comments, settings and links
	 *
	 * @access	public
	 * @param	string
	 * @return	object
	...
	{
		$model_name = 'companies_'.strtolower($model).'_model';
	...
	function get_logo_list()
	{
		...
		$list_a=$ci->companies_packages_subscriptions_model->find_all_array($where);
		if(!empty($list_a))
		{
		...
		$list_b=$this->CI->companies_features_subscriptions_model->find_all_array($where);
		if(!empty($list_b))
		{
	...
	function items_list($term, $my_array, $order_by = 'tbl_institutions.institution_name asc', $limit = NULL, $offset = NULL)
	{
		...
		$res=$this->CI->companies_institutions_model->get_search_data($term, $my_array, $order_by, $limit, $offset);
		return $res; 
	}
	
	function course_items_list($term, $my_array, $order_by = 'tbl_courses.course_title asc', $limit = NULL, $offset = NULL)
	{
		$res=$this->CI->companies_courses_model->get_search_data($term, $my_array, $order_by, $limit, $offset);
		return $res; 
	}
	...
	{
		$res=$this->CI->companies_international_universities_model->get_search_data($term, $my_array, $order_by, $limit, $offset);
		return $res; 
	}
	function campuses_by_institution($where)
	{
		return $this->CI->companies_campuses_model->campuses_by_institution($where);
	}
	function courses_by_institution($where)
	{
		return $this->CI->companies_courses_model->courses_by_institution($where);
	}
	function get_counties($where)
	{
		...
		if($val>0)
		{
			$res=$this->CI->companies_countries_model->find_one_array($where);
			$ctry=$res['slug'];
			$county="";
			if($ctry==="kenya")
			{
				$filter=array('slug != '=>'international');
				$county=$this->CI->companies_counties_model->counties_ajax($filter);
			}
			else
			{
				$filter=array('slug'=>'international');
		...
		{
			$county=$this->CI->companies_counties_model->counties_ajax(array());
		}
	...
	function get_counties_opts($where)
	{
		...
		{
			$filter=array('slug != '=>'international');
			$county=$this->CI->companies_counties_model->options_list('id','name',$filter);
		}
		else
		{
			$filter=array('slug'=>'international');
	...
	{
		return $this->CI->companies_institutions_model->find_one_array($filter);
	}
	function get_campuses($filter=array())
	{
		return $this->CI->companies_campuses_model->find_all_array($filter);
	}
	function get_courses($filter)
	{
		return $this->CI->companies_courses_model->find_all_array($filter);
	}

	function update_inst_profile($post,$where)
	{
		$model='companies_institutions_model';
	...
	{
			$res=$this->CI->companies_institutions_model->find_one_array(array("institution_id"=>$where['institution_id']));
			...
			$params['from_name']=$this->fuel->companies->config('email_nice_name');
    ...
    public function update_account_password($data,$where)
	{
		$where['is_active'] = 'yes';
		$model="companies_institutions_model";
		$lib='companies_institutions_auth';
	...
	public function get_features()
	{
		...
		return $this->CI->companies_features_model->find_all_array($filter);
	}
	...
	{
		$filter=array('published'=>'yes');
		return $this->CI->companies_packages_model->find_all_array($filter);		
	}
	...
	public function get_my_features($where)
	{
		...
		return $this->CI->companies_features_subscriptions_model->find_all_array($where);
	}
	...
	public function get_my_packages($where)
	{
		$now=datetime_now();
		$where['expiry_date >= ']=$now;
		return $this->CI->companies_packages_subscriptions_model->find_all_array($where);		
	}
	...
	{
		return $this->CI->companies_international_universities_model->find_all_array();
	}
	...
	{
		$res=$this->CI->companies_institutions_model->find_one_array($where);
		$slug=$res['slug'];		
	...
	{
		$model='companies_international_universities_model';
		$data=$this->CI->$model->on_before_clean($data);
		$data=$this->CI->$model->clean($data);
		$data=$this->CI->$model->on_before_save($data);
		$data=$this->CI->$model->insert($data);
	...
	{
		$course_dets=$this->CI->companies_courses_model->get_search_data("",array('tbl_courses.course_id'=>$filter['course_id']));
		$course=$course_dets[0];

		$campuses=create_array($this->CI->companies_course_to_campuses_model->options_list('campus_id','campus_id',$filter));
		$locations=create_array($this->CI->companies_course_to_locations_model->options_list('location_id','location_id',$filter));
		$course_sectors=create_array($this->CI->companies_course_to_sectors_model->options_list('sector_id','sector_id',$filter));
		$class_hours=create_array($this->CI->companies_course_to_class_types_model->options_list('class_type_id','class_type_id',$filter));
		$course_types=create_array($this->CI->companies_course_to_course_types_model->options_list('course_type_id','course_type_id',$filter));
		$course_levels=create_array($this->CI->companies_course_to_levels_model->options_list('course_level_id','course_level_id',$filter));
	...
	{
		$university_dets=$this->CI->companies_international_universities_model->get_search_data("",array('tbl_international_universities.institution_id'=>$filter['institution_id']));
		$university=$university_dets[0];

		$faculties=create_array($this->CI->companies_international_universities_to_faculty_model->options_list('faculty_id','faculty_id',$filter));
		
		$qualifications=create_array($this->CI->companies_international_universities_to_qualifications_model->options_list('qualification_id','qualification_id',$filter));
	...
	public function get_my_free_slots($where)
	{
		...
		if(!empty($pack))
		{
			...
			$res=$this->CI->companies_packages_model->find_one_array($filter);
	...
	{
		$model='companies_form_downloads_model';
		$data=$this->CI->$model->on_before_clean($data);
		$data=$this->CI->$model->clean($data);
	...
	{
		$model='companies_quick_downloads_log_model';
		$data=$this->CI->$model->on_before_clean($data);
		$data=$this->CI->$model->clean($data);
		$this->CI->$model->insert($data);
		return $data;
	...
	{
		return $this->CI->companies_form_downloads_model->find_one_array($where);
	}
	function get_inquiry_details($where)
	{
		return $this->CI->companies_university_inquiries_model->find_one_array($where);
	}
	function get_quick_downloads($where)
	{
		return $this->CI->companies_quick_downloads_model->find_all_array($where);
	}
	function get_parent_inst($where)
	{	
		$res= $this->CI->companies_campuses_model->find_one_array($where); 
		return $res['institution_id'];
	}

	function get_hot_courses()
	{	
		$opts=$this->CI->companies_hot_courses_bindings_model->options_list('course_id','course_id'); 

		$res=array();
		foreach ($opts as $key => $value) 
		{
			$filter=array('course_id'=>$value);
			$res[]= $this->CI->companies_courses_model->find_one_array($filter); 	
		}
		shuffle($res);
		return $res;
	}
	...
	{
		//$this->CI->companies_courses_model->migrate_courses();
		//return;
	}
	function inst_types()
	{
		return $this->CI->companies_institution_types_model->options_list();
	}
	function migrate_alerts()
	{
		//$this->CI->companies_course_alerts_model->migrate_alerts();
		//return;
	}
	public function add_campus($data)
	{
		$model='companies_campuses_model';
		$data=$this->CI->$model->on_before_clean($data);
		$data=$this->CI->$model->clean($data);
		$data=$this->CI->$model->on_before_save($data);
		$data=$this->CI->$model->insert($data);		
	...
	{
		$model='companies_university_inquiries_model';
		$data=$this->CI->$model->on_before_clean($data);
		$data=$this->CI->$model->clean($data);
		$data=$this->CI->$model->insert($data);		
		return;
